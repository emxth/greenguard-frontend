...
function CreateSchedule() {
    ...
    useEffect(() => {
        if (pickUpID) {
            axios.get(`http://localhost:8070/pickupRequests/getPickupInfo/${pickUpID}`)
                .then((response) => {
    ...
    const sendData = async (e) => {
        ...
        try {
            await axios.post("http://localhost:8070/shedulePickup/addPickUpSchedule", newPickUpSchedule);
            await axios.put(`http://localhost:8070/requestTruck/updateStatus/${truckReqID}`, {
            ...
            if (sendSMS && pickupTell) {
                const smsMessage =
                    ...
                    `Date: ${scheduleDate}\n` +
                    `Time: ${formattedTime}`;
                await axios.post("http://localhost:8070/sms/send", {
                    to: '+94' + pickupTell,
                    message: smsMessage
                });
            }
